...
class EKF_SLAM:
    def __init__(self):
        ...
        self.final_covariance = np.eye(3)  # Initial covariance matrix
        ...
        self.covariance = np.eye(3)  # Initial covariance
    ...
    def prediction_covariance_calc_for_robot(self):  
        ...
        trans_variance = alpha3 * abs(self.delta_trans) + alpha4 * (abs(self.delta_rot1) + abs(self.delta_rot2))
    ...
    def lidar_callback(self,msg):
        ...
        for i in range(0,len(self.lidar_points)):
            if(math.isnan(self.lidar_points[i]) or math.isinf(self.lidar_points[i])):
    ...
    def final_prediction(self):
        self.covariance = np.dot(self.G_resultant,np.dot(self.covariance,self.G_resultant.T)) + self.R_resultant
